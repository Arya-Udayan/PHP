...
public class OnlineCourseRegistrationSystem {
    private static final String DB_URL = "jdbc:mysql://localhost:3306/db_ocr?characterEncoding=utf8";
    ...
    public static void main(String[] args) {
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            ...
            initializeDummyData(conn); // Call initializeDummyData only once here
    ...
    private static void enrollInCourse(Connection conn, String username, Scanner scanner) throws SQLException {
        ...
        String insertEnrollmentSQL = "INSERT INTO enrollments (username, course_id) VALUES (?, ?)";
    ...
    private static void initializeDummyData(Connection conn) throws SQLException {
        // Check if courses already exist before inserting dummy data
        try (Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT COUNT(*) AS count FROM courses")) {
            rs.next();
            int count = rs.getInt("count");
            if (count == 0) { // If no courses exist, then insert dummy data
